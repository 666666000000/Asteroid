...
def resolve(line,isReturn):
	...
	if arg[0] == "play":
		...
		if not l:
			return main.setEntry(f"未找到:{line}")
...
def loadList(name,start = False):
	if not on:
		return main.setEntry("VLC未启动")
	inputpath = core.getInputPath(name)
	if not inputpath:
		return main.setEntry("未找到文件")
	path = ""
	for p in inputpath:
...
def turnOn(arg,argLen):
	...
	if playerError:
		return main.setEntry("播放器错误")
	if on:
		return main.setEntry("VLC已启动")
