...
def encode(arg,argLen):
	...
	if argLen <= 3 :
		if argLen == 3:
			dstPath = core.getOutputDirPath(arg[2],"None","first","checkDir","file")
		showBarcode(arg[1])
	elif argLen >= 4:
		if arg[2] == "*":
...
def showBarcode(arg):
	global barImg
	img = barCodeImage(arg,core.getOriClipboard().strip())
	if not img:
		return
	window = Toplevel(bg = "white")
	window.overrideredirect(True)
	window.geometry(f"{size}x{size}+{xPos}+{yPos}")
	height = floor(size*img.height/img.width)
	resized = img.resize((size,height),Image.ANTIALIAS)
	barImg = ImageTk.PhotoImage(resized)
	ypos = floor(size-height)/2
	imgLabel = Label(window,image = barImg)
	imgLabel.place(y = ypos,width = size , height = height)
